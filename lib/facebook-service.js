...
FacebookService.handle = function *(data, _user) {
  let fb   = yield this.getFacebookProfile(data, data.redirectUri);
  let user = null;
  switch (data.type) {
    case 'login' :
 ...
 */
FacebookService.getFacebookProfile = function *(data, redirectUri) {
  let facebook = new Facebook();
  let token    = data.code ? yield facebook.getToken(code, redirectUri) : data.token;
...
FacebookService.verifyData = function (data) {
  ...
  if (!data.code && !data.token)        { errors.push('code or token'); }
  if (!data.token && !data.redirectUri) { errors.push('redirectUri'); }
  if (errors.length) {
    throw error.parse({
